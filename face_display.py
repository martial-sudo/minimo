...
class FaceDisplay:
    ...
    def _update_display(self):
        """Update the display based on current emotion and time of day"""
        ...
        if is_night_time:
            # For night time, completely modify the display to show time
            # Clear screen with a dark background
            dark_background = pygame.Color(0, 0, 0)  # Black background
            self.screen.fill(dark_background)
            
            # Draw the clock prominently
            self._draw_night_time_clock()
        else:
            # Regular daytime face display
    ...
    def _draw_night_time_clock(self):
        """Draw a clock showing the current time during night hours"""
        current_time = datetime.now().strftime("%H:%M")
        current_date = datetime.now().strftime("%B %d, %Y")
        
        # Set up the font for the clock
        try:
            # Try to use a system font
            time_font = pygame.font.SysFont("Arial", 120, bold=True)
        ...
        time_surface = time_font.render(current_time, True, pygame.Color(255,255,255))
        date_surface = date_font.render(current_date, True, pygame.Color(0,255,0))
        
        # Position the time and date
        time_rect = time_surface.get_rect(center=(self.screen_width // 2, self.screen_height // 2 - 30))
        date_rect = date_surface.get_rect(center=(self.screen_width // 2, self.screen_height // 2 + 70))
        
        # Draw the time and date on the screen
    ...
    #     # Render the time text
    #     time_surface = font.render(current_time, True, pygame.Color("white"))
    #     time_rect = time_surface.get_rect(center=(self.screen_width // 2, self.screen_height // 4))
